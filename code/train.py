...
def train_model():
    ...
    gnb_AUC_max = -1
    for i in range(1000):
        X, y = load_dataset(r"temp_data\train_data.csv")
        ...
        if knn_AUC_max < roc_auc_score(y_test, knn_predict_proba.max(axis=1)):
            ...
            joblib.dump(knn, "code/model/knn_model.h5")
        ...
        if dt_AUC_max < roc_auc_score(y_test, dt_predict_proba.max(axis=1)):
            ...
            joblib.dump(dt, "code/model/dt_model.h5")
        ...
        if gnb_AUC_max < roc_auc_score(y_test, gnb_predict_proba.max(axis=1)):
            ...
            joblib.dump(gnb, "code/model/gnb_model.h5")
